...
class SavvyStreamingAPI(savvy_streaming_api_pb2_grpc.SavvyStreamingAPIServicer):
    ...
    def terminationCallback():
        # TODO raise and exception, catch it in the caller and return from StreamMachine
        print('Termination callback called')
        return
    ...
    def StreamMachine(self, request, context):
        ...
        # TODO it is not being called! Suffering this bug https://stackoverflow.com/questions/44243099/python-detect-client-drop-during-grpc-infinite-streamç
        # https://github.com/grpc/grpc/issues/11157
        context.add_callback(self.terminationCallback)
        ...
        if self.JWT_is_valid(request, context, accepted_roles):
            ...
            while True:
                ...
                line = now.strftime("%B %d, %Y")
                print ('Devolviendo: ' + line)
